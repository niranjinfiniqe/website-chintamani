...
class MysqliDb
{
   ...
   public function query($query, $numRows = null)
    ...
    // public function addImages($imagename)
    // {
    //     $target_dir = "uploads/";
     ...
     * @return bool Boolean indicating whether the insert query was completed succesfully.
     */
    private function _buildInsert($tableName, array $insertData, $operation)
    {
        if ($this->isSubQuery) {
            return;
        }
        ...
        $stmt = $this->_buildQuery($tableName, $insertData);
        $status = $stmt->execute();
        $this->_stmtError = $stmt->error;
        $this->_stmtErrno = $stmt->errno;
    ...
    protected function _prepareQuery()
    {
        ...
        $errno = $this->mysqli()->errno;
        $this->reset();
        throw new Exception(sprintf('%s query687y2y383293983: %s', $error, $query), $errno);
    }
